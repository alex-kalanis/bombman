...
class MapTile(object):
    ...
    def shouldnt_walk(self):
        return self.kind in [MapTile.TILE_WALL, MapTile.TILE_BLOCK] or len(
            self.flames) >= 1 or self.special_object == MapTile.SPECIAL_OBJECT_LAVA
...
class GameMap(object):
        ...
        for bomb in self.bombs:
            ...
            for i in range(bomb.flame_length):
                for direction in (0, 1, 2, 3):
                    ...
                    if not self.tile_is_walkable(position[direction]) or not self.tile_is_withing_map(
                            position[direction]):
    ...
    def get_players_at_tile(self, tile_coordinates):
        ...
        for player in self.players:
            ...
            if not player.is_dead() and not player.is_in_air() and player_tile_position[0] == tile_coordinates[0] and \
                    player_tile_position[1] == tile_coordinates[1]:
    ...
    def tile_is_withing_map(self, tile_coordinates):
        return tile_coordinates[0] >= 0 and tile_coordinates[1] >= 0 and tile_coordinates[
            0] <= GameMap.MAP_WIDTH - 1 and tile_coordinates[1] <= GameMap.MAP_HEIGHT - 1
    ...
    def tile_is_walkable(self, tile_coordinates):
        ...
        return self.tile_is_withing_map(tile_coordinates) and (self.tiles[tile_coordinates[1]][tile_coordinates[
            0]].kind == MapTile.TILE_FLOOR or tile.to_be_destroyed) and not self.tile_has_bomb(tile_coordinates)
    ...
    def spread_items(self, items):
        ...
        for y in range(GameMap.MAP_HEIGHT):
            for x in range(GameMap.MAP_WIDTH):
                ...
                if tile.kind == MapTile.TILE_FLOOR and tile.special_object == None and tile.item == None and not self.tile_has_player(
                        (x, y)):
...
class Menu(object):
    ...
    def prompt_if_needed(self, menu_item_coordinates):
        if self.state == Menu.MENU_STATE_CONFIRM and (
                self.confirm_prompt_result == None or self.confirm_prompt_result == False) and self.selected_item == menu_item_coordinates:
...
class AI(object):
    ...
    def number_of_blocks_next_to_tile(self, tile_coordinates):
        ...
        for tile_offset in ((0, -1), (1, 0), (0, 1), (-1, 0)):  # for each neigbour file
            helper_tile = self.game_map.get_tile_at(
                (tile_coordinates[0] + tile_offset[0], tile_coordinates[1] + tile_offset[1]))
...
class Game(object):
    ...
    def filter_out_disallowed_actions(self, actions):
        player_slots = self.play_setup.get_slots()
        result = filter(
            lambda a: (player_slots[a[0]] != None and player_slots[a[0]] >= 0) or (a[1] == PlayerKeyMaps.ACTION_MENU),
            actions)
        return result
